...
class ElectraForQuestionAnswering21(ElectraPreTrainedModel):
    ...
    def __init__(self, config, the_device='cpu', num_decoupling=1):
        ...
        self.relation_embeds = nn.Embedding(18, config.hidden_size) #0-15relations, 16cls, 17padding
        self.dGCN = RGCNModel(config.hidden_size, 6, 2, True)
        self.sGCN = RGCNModel(config.hidden_size, 3, 1, True)
        self.qa_outputs = nn.Linear(4*config.hidden_size, config.num_labels)
        self.has_ans = nn.Linear(config.hidden_size, 2)
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        # print("sequence_output:",sequence_output.size())
        sa_self_word_level ,selfa= self.SASelfMHA[0](sequence_output, sequence_output, attention_mask = sa_self_mask, output_attentions=True)#[0]
        sa_cross_word_level,crossa = self.SACrossMHA[0](sequence_output, sequence_output, attention_mask = sa_cross_mask, output_attentions=True)#[0]
        for t in range(1, self.num_decoupling):
            sa_self_word_level,selfa = self.SASelfMHA[t](sa_self_word_level, sa_self_word_level, attention_mask = sa_self_mask, output_attentions=True)#[0]
            sa_cross_word_level,crossa = self.SACrossMHA[t](sa_cross_word_level, sa_cross_word_level, attention_mask = sa_cross_mask, output_attentions=True)#[0]
        sa_word_level = self.fuse2(sequence_output, sa_self_word_level, sa_cross_word_level)
        torch.save(selfa,'selfa.pt')
        torch.save(crossa, 'crossa.pt')
        ...
        # print('gs')
        torch.save(X2, 'X2.pt')
        torch.save(sa_self_word_level, 'self.pt')
        torch.save(sa_cross_word_level, 'cross.pt')
        torch.save(sequence_output,'context.pt')
        # print('mask')
        torch.save(sa_word_level, 'sa_word_level.pt')
        # print('logits')
        torch.save(start_logits, 'start.pt')
        torch.save(end_logits, 'end.pt')
        if not return_dict:
            output = (start_logits, end_logits, has_log) + outputs[2:]
            return ((total_loss,) + output) if total_loss is not None else output
...
class ElectraForQuestionAnswering33(ElectraPreTrainedModel):
...
class ElectraForQuestionAnswering31(ElectraPreTrainedModel):

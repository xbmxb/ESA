...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config, the_device='cpu'):
        ...
        self.relation_embeds = nn.Embedding(2, config.hidden_size) #0-15relations, 16cls, 17padding #0cls 1 padding
        self.GCN = RGCNModel(config.hidden_size, 3, 1, True)
    ...
    def forward(
        ...
        for batchi in range(batch_size):
            ...
            for id in range(1,len(sep_pos)):
                speaker_dic[id-1]=speaker_ids[batchi][sep_pos[id]]
            # print("speaker_ids",speaker_ids[batchi])
            ...
            for item in range(num_utterance):
                for item1 in range(item+1,num_utterance):
                    ...
                    # print("item,item1", item,item1)
                    if speaker_dic[item]==speaker_dic[item1]:
                        G.add_edges(item, item1)
                        edges.append([item, item1])
                        edge_type.append(0)
                        G.add_edges(item1, item)
            ...
            for i in range(num_utterance + 1):
                if i < num_utterance:
                    ...
                    G.nodes[[i]].data['h'] = self.relation_embeds(Variable(torch.LongTensor([0,]).to(self.the_device)))
                # else:
            ...
            for i in range(0, len(sep_pos)):
                if i==0:
                    for j in range(sep_pos[i]):
                        app[j+1] = self.relation_embeds(Variable(torch.LongTensor([1,])).to(self.the_device))
            ...
            for i in range(seq_len-sep_pos[-1]-1):
                app[i+sep_pos[-1]+1] = self.relation_embeds(Variable(torch.LongTensor([1,])).to(self.the_device))
...
class ElectraForQuestionAnswering(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        for batchi in range(batch_size):
            ...
            sep_embedding = torch.ones(num_utterance, hidden_size).to(self.the_device)
            for sep_idx in range(len(sep_pos)-1):
                sep_embedding[sep_idx] = sequence_output[batchi,sep_pos[sep_idx+1],:]#???
            for id in range(num_utterance):
                speaker_dic[id]=speaker_ids[batchi][sep_pos[id+1]]
            G = dgl.DGLGraph()
            G.to(self.the_device)
            relation = []
            edge_type = []    # in total six type of edges

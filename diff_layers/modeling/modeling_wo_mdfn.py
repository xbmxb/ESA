...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        # word_level = self.dropout(sa_word_level)
        ...
        for batchi in range(batch_size):
            ...
            for i in range(num_utterance + 1):
                if i < num_utterance:
                    ...
                    sG.nodes[[i]].data['h'] = self.relation_embeds(Variable(torch.LongTensor([0,]).to(self.the_device)))
                # else:
                #     index_relation = relation[i-num_utterance-1]
            ...
            for i in range(0, len(sep_pos)):
                if i==0:
                    for j in range(sep_pos[i]):
                        app1[j+1] = self.relation_embeds(Variable(torch.LongTensor([1,])).to(self.the_device))
                        # print("j+1,padding",j+1, app[j+1])
                else:
                    for j in range(sep_pos[i-1], sep_pos[i]):
                        app1[j+1] = X2[i-1]
                        # print("j+1,X[i]",j+1, X[i-1])
            for i in range(seq_len-sep_pos[-1]-1):
                app1[i+sep_pos[-1]+1] = self.relation_embeds(Variable(torch.LongTensor([1,])).to(self.the_device))
            #     print("i,padding",i+sep_pos[-1]+1,app[i+sep_pos[-1]+1])
            # print("app", app)
            app01[batchi,:,:] = app1[:,:]
        #cat the matrix and sequeence_output
        
        # with_gcn = torch.cat((word_level,app0,app01), 2)
        with_gcn = torch.cat((sequence_output,app0,app01), 2)

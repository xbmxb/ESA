...
def squad_convert_examples_to_features(
    ...
    if return_dataset == "pt":
        ...
        all_relations = torch.tensor([f.relations for f in features], dtype=torch.long)
        ...
        if not is_training:
            ...
            dataset = TensorDataset(
                all_input_ids, all_attention_masks, all_token_type_ids, all_feature_index, all_cls_index, all_p_mask, all_relations, all_speaker_ids, all_sep_position, all_turn_ids
            )
        else:
...
class SquadExample:
    ...
    def __init__(
        ...
        #relations matrix
        relations_matrix = [[-1 for i in range(3)] for i in range(13)]
        for idx in range(len(relations)):
            if relations[idx]['type'] not in relation_key_pair.keys():
                continue
            relations_matrix[idx][0] = relations[idx]['y']
            relations_matrix[idx][1] = relations[idx]['x']
            relations_matrix[idx][2] = relation_key_pair[relations[idx]['type']]
